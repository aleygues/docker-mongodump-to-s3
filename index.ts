...
async function dump() {
  // get all containers ids with fr.aleygues.mongodump label
  let containersIds: string[] = [];
  {
    const output = (await sh(
      "docker ps -q -f 'label=fr.aleygues.mongodump'",
      "."
    )) as string;
  ...
  }[] = [];
  {
    ...
    for (const container of json) {
      ...
      if ("fr.aleygues.mongodump" in labels === false) {
        console.warn(`Container ${container.Id} does not has mongodump label`);
        continue;
      }

      if (
        typeof labels["fr.aleygues.mongodump.prefix"] !== "string" ||
        !labels["fr.aleygues.mongodump.prefix"]
      ) {
        console.error(
          `Container ${container.Id} must have mongodump.prefix label`
        );
        continue;
      }

      const prefix = labels["fr.aleygues.mongodump.prefix"];
      ...
      if (
        typeof labels["fr.aleygues.mongodump.frequency"] === "string" &&
        ["daily", "hourly", "weekly"].includes(
          labels["fr.aleygues.mongodump.frequency"]
        ) === false
      ) {
        console.error(
          `Container ${container.Id} uses a frequency that does not exist (${labels["fr.aleygues.mongodump.frequency"]})`
        );
        continue;
      }

      if (
        typeof labels["fr.aleygues.mongodump.daysRetention"] === "string" &&
        /^[0-9]+$/.test(labels["fr.aleygues.mongodump.daysRetention"]) === false
      ) {
        console.error(
          `Container ${container.Id} uses an invalid mongodump.daysRetention value`
        );
        continue;
      }
      ...
      containersData.push({
        id: container.Id,
        prefix: labels["fr.aleygues.mongodump.prefix"],
        frequency: labels["fr.aleygues.mongodump.frequency"] || "daily",
        daysRetention: labels["fr.aleygues.mongodump.daysRetention"]
          ? Number(labels["fr.aleygues.mongodump.daysRetention"])
          : 7,
      });
    }
  }
  ...
  for (const container of containersData) {
    ...
    const searchKey = `mongodump_${container.prefix}_${format(
      new Date(),
    ...
    try {
      const path = `/dumps/mongodump_${container.prefix}_${format(
        new Date(),
        DATETIME_FORMAT
      )}.json`;
      await sh(
        `docker exec ${container.id} sh -c 'mongodump --archive' > ${path}`,
        "."
      );
      console.log(
      ...
      try {
        ...
        try {
          ...
          console.log(
            `Dumps starting with mongodump_${container.prefix} and older than ${container.daysRetention} days has been deleted`
          );
        } catch {
          console.error(
...
async function clearDumps(
  ...
  const fullPrefixWithUnderscore = `mongodump_${prefix}_`;
